...
// InstancedMesh-per-class occupancy viewer.
(function () {
  ...
  class OccViewer {
    constructor(container, occUrl, opts = {}) {
      ...
      this._showStatic = !(opts && opts.showStatic === false);  // false => agents only (background hidden)
      this.staticMeshes = [];
      this.dynMeshes = [];
      this.frame = 0;
      this.playing = false;
      ...
      this._pending = null;
      this._build();
      fetch(occUrl).then((r) => r.json()).then((d) => { if (this._alive) this._init(d); }).catch((e) => {
        if (!this._alive) return;                 // disposed mid-flight (rapid scene switch) -> no leaked context
    ...
    // show/hide the static background voxels (class 1). Safe to call before load — applied in _init.
    setStaticVisible(v) { this._showStatic = v; this.staticMeshes.forEach((m) => (m.visible = v)); }
    ...
    _init(d) {
      ...
      if (!this._showStatic) this.staticMeshes.forEach((m) => (m.visible = false));  // agents-only toggle
    ...
    dispose() {
      ...
      try {
        ...
        if (this.renderer) {
          ...
          this.renderer.forceContextLoss && this.renderer.forceContextLoss();  // release GL context now, not at GC
